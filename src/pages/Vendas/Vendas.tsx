...
import {
  FORMAS_PAGAMENTO,
} from '@/constants/formasPagamentos'
...
export function Vendas() {
  ...
  const estoqueSelecionado =
    estoqueAtual.find(
      (e) => e.corte === form.corte
    )

  const saldoDisponivel =
    Number(
      estoqueSelecionado?.saldo_liquido_kg ||
      0
    )

  async function carregarClientes() {
    const data =
      await clientesService.getOptions()
  ...
  async function carregarEstoque() {
    const data =
      await estoqueService.getEstoqueAtual()
  ...
  async function handleCreate() {
    try {
      ...
      if (
        ...
        toast.error(
          `Estoque insuficiente. Disponível: ${saldoDisponivel} kg`
        )
  ...
  async function handleUpdate() {
    try {
      ...
      await vendasService.update(
        editar.id,
        payload
      )

      const stockMovement =
        await estoqueService.getByLote(
          `v-${editar.id}`
        )
      ...
      if (stockMovement) {
        const shouldUpdate =
          ...
          editar.corte ||
          Number(
            stockMovement.peso_liquido_kg
          ) !==
          Number(
            editar.peso_kg
          ) ||
          stockMovement.data_movimentacao !==
          editar.data_venda
  ...
  const columns = [
...

    {
      ...
      render: (r: VendaRow) =>
        STATUS_VENDA_LABEL[
        r.status as keyof typeof STATUS_VENDA_LABEL
        ],
    },
  ...
  return (
    <div>
      ...
      <Card title="Nova venda">
        ...
        <Button
          disabled={loading || form.cliente_id === '' || form.corte === '' || form.forma_pagamento === ''}
